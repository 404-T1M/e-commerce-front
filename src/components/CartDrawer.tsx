...
import { X, ShoppingCart, Trash2, Plus, Minus, ArrowRight, ShoppingBag, AlertCircle } from 'lucide-react';
...
export function CartDrawer() {
    const { items, count, totalPrice, finalTotalPrice, isOpen, closeCart, updateQuantity, removeItem, isLoading } = useCart();
    ...
    return (
        <>
            ...
            <div
                ref={overlayRef}
                onClick={closeCart}
                className={`fixed inset-0 z-[60] bg-black/40 backdrop-blur-sm transition-none ${isOpen ? 'pointer-events-auto' : 'pointer-events-none opacity-0'}`}
            />
            {/* Drawer */}
            <div
                ref={drawerRef}
                className="fixed right-0 top-0 h-full z-[70] w-full max-w-sm bg-white shadow-2xl flex flex-col translate-x-full"
            >
                {/* Header */}
                <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                    ...
                    <button onClick={closeCart} className="p-2 rounded-xl hover:bg-gray-50 text-gray-500 hover:text-gray-700 transition-colors">
                        <X className="w-5 h-5" />
                    </button>
                </div>
                    ...
                    {isLoading ? (
                        ...
                        <div className="space-y-4">
                            {items.map((item) => {
                                ...
                                return (
                                    <div key={String(item.variantId)} className={`flex gap-3 group ${isUnavailable ? 'opacity-60' : ''}`}>
                                        {/* Image */}
                                        ...
                                        <p className="text-sm font-bold text-brand-600 mt-0.5">
                                            ...
                                            <p className="text-xs text-gray-400">{formatCurrency(unitPrice, currency, intlLocale)} {copy.each}</p>

                                            {/* Qty controls */}
                                            <div className="flex items-center gap-2 mt-2">
                                                <button
                                                    onClick={() => handleUpdateQty(String(item.variantId), item.quantity - 1)}
                        ...
                        <div className="space-y-1.5">
                            {hasDiscount && (
                                <div className="flex items-center justify-between text-sm">
                                    ...
                                    <span className="text-gray-400 line-through">{formatCurrency(totalPrice, currency, intlLocale)}</span>
                                </div>
                            )}
                            <div className="flex items-center justify-between">
                                ...
                                <span className="text-xl font-bold text-gray-900">{formatCurrency(finalTotalPrice, currency, intlLocale)}</span>
                            </div>
                        </div>
