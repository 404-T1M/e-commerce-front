...
export function useCategories() {
  ...
  const setFilterPublished = (v: string) => {
    setSearchParams(
      (prev) => {
        if (v) prev.set("published", v);
        else prev.delete("published");
        prev.set("page", "1");
        return prev;
      },
      { replace: true },
  ...
  const setParent = (parentId: string) => {
    setSearchParams(
      (prev) => {
        if (parentId) prev.set("parent", parentId);
        else prev.delete("parent");
        prev.set("page", "1");
        return prev;
      },
      { replace: true },
  ...
  const query = useQuery({
    ...
    queryFn: () =>
      categoriesApi.list({
        ...
        published:
          filterPublished !== "" ? filterPublished === "true" : undefined,
