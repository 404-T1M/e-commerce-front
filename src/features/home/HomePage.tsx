  ...
  const copy =
    locale === "ar"
      ...
      : {
          ...
          subheading:
            "Curated collections designed to elevate your everyday essentials.",
          shopNow: "Shop Now",
      ...
      <div className="group relative overflow-hidden rounded-3xl aspect-square bg-ink-50 cursor-pointer border border-ink-100 hover:shadow-card-hover transition-all duration-500 hover:-translate-y-1">
        ...
        <div className="absolute inset-x-0 bottom-0 p-4">
          <div className="bg-white/90 backdrop-blur-sm rounded-2xl px-3 py-2">
            <p className="text-ink-800 font-semibold text-sm truncate">
              {name}
            </p>
          </div>
        </div>
...
function ProductMiniCard({ product }: { product: SectionProduct }) {
  ...
  const copy =
    locale === "ar" ? { view: "عرض التفاصيل" } : { view: "View details" };
...
function SectionRenderer({ section }: { section: Section }) {
  ...
  switch (section.type) {
    case "hero_banner":
      return data.banners?.length ? (
        <BannerHero banners={data.banners} />
      ) : null;
    case "slider":
      return data.banners?.length ? (
    ...
    case "customCategoriesSection":
      return (
        <section className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <SectionHeader
            title={title}
            description={description || t("explore")}
            actionHref="/products"
