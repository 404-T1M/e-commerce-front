...
import {
    BarChart2, ShoppingBag, CheckCircle2, Clock, Users,
    MessageSquare, TrendingUp, RefreshCw, ChevronDown,
    BarChart as BarChartIcon, Activity, ArrowUpRight,
} from 'lucide-react';
import { analyticsApi } from '@/api/analytics.api';
import { cn, DEFAULT_CURRENCY, formatCompactCurrency, formatCurrency, getIntlLocale } from '@/utils';
        ...
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="page-header">
                ...
                <div className="flex items-center gap-3">
                    <div className="flex items-center gap-2 text-xs text-slate-400 bg-white border border-slate-200 px-3 py-2 rounded-xl">
                        <ArrowUpRight className="w-3.5 h-3.5 text-emerald-500" /> {copy.liveData}
                    </div>
            ...
            <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4">
                    {STATS.map((s) => <StatCard key={s.label} {...s} loading={overviewLoading} />)}
                </div>
            </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-6">
                    <div className="flex-1">
                        <h2 className="flex items-center gap-2 text-base font-bold text-slate-800">
                            <TrendingUp className="w-5 h-5 text-brand-600" /> {copy.chartTitle}
                        </h2>
                    ...
                    <ResponsiveContainer width="100%" height={288}>
                        {chartType === 'area' ? (
                            <AreaChart data={chartPoints} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
                                ...
                                <Area type="monotone" dataKey="revenue" name={copy.revenue} stroke="#6366f1" strokeWidth={2.5} fill="url(#ovGradRev)" dot={false} activeDot={{ r: 5, strokeWidth: 0 }} />
                                <Area type="monotone" dataKey="profit"  name={copy.profit}  stroke="#10b981" strokeWidth={2.5} fill="url(#ovGradPro)" dot={false} activeDot={{ r: 5, strokeWidth: 0 }} />
                            </AreaChart>
                        ) : (
