...
import { Wallet, ArrowDownLeft, ArrowUpRight, Clock, Plus, AlertCircle } from 'lucide-react';
import { walletApi, type WalletTransaction } from '@/api/wallet.api';
import { Pagination } from '@/components/DataTable';
import { AccessDeniedState } from '@/components/AccessDeniedState';
import { cn, DEFAULT_CURRENCY, formatCurrency, formatDate, getIntlLocale, isForbiddenError } from '@/utils';
...
export function UserWalletTab({ userId }: { userId: string }) {
    ...
    const { data: txData, isLoading: txLoading, error } = useQuery({
    ...
    const meta: any = txData?.meta || txData?.pagination;
    
    // Estimate balance from the most recent transaction on the first page
    const estimatedBalance = page === 1 && transactions.length > 0 ? transactions[0].balanceAfter : null;

    const creditMutation = useMutation({
        mutationFn: (data: { amount: number; note: string }) => walletApi.adminCreditWallet({ userId, ...data }),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['adminUserTransactions', userId] });
            toast(copy.toast.credited, 'success');
            setIsModalOpen(false);
        },
        onError: (err: any) => {
            toast(err?.response?.data?.message || copy.toast.creditFailed, 'error');
        }
    });
        ...
        <div className="space-y-6">
                ...
                <div className="divide-y divide-gray-100">
                    {txLoading ? (
                        ...
                        transactions.map((tx: WalletTransaction) => {
                            ...
                            const txLabel = tx.note || copy.tx[tx.type as keyof typeof copy.tx] || copy.tx.purchase;
                            return (
                            <div key={tx.id} className="p-4 sm:px-6 flex items-center gap-4 hover:bg-gray-50/50 transition-colors">
                                ...
                                <div className="text-right shrink-0">
                                    ...
                                    <p className="text-xs text-gray-400 mt-0.5">
                                        {copy.balance}: {formatCurrency(tx.balanceAfter, currency, intlLocale)}
                                    </p>
                                </div>
                            </div>
                        )})
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-fade-in">
                    <form onSubmit={handleCreditSubmit} className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-slide-up">
                        ...
                        <div className="p-6 space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">{copy.modal.amount}</label>
                                <input 
                                    name="amount" 
                                    type="number" 
                                    min="0.01" 
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">{copy.modal.note}</label>
                                <input 
                                    name="note" 
                                    type="text" 
                        ...
                        <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex justify-end gap-2">
                            <button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 rounded-xl transition-colors">
                                {copy.modal.cancel}
                            </button>
                            <button type="submit" disabled={creditMutation.isPending} className="px-4 py-2 text-sm font-medium bg-brand-600 text-white rounded-xl hover:bg-brand-700 transition-colors disabled:opacity-50">
                                {creditMutation.isPending ? copy.modal.confirming : copy.modal.confirm}
                            </button>
                        </div>
                    </form>
                </div>
