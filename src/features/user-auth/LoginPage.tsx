...
import { z } from 'zod';
import { Mail, Lock, Eye, EyeOff, ArrowRight, Phone, ChevronDown } from 'lucide-react';
    ...
    const onSubmit = async (data: FormValues) => {
        ...
        if (loginMethod === 'email') {
            ...
            if (!data.mobilePhone?.trim()) {
                setError('mobilePhone', { type: 'manual', message: copy.phoneRequired });
                return;
            }
    ...
    return (
        <>
            <AuthSplitShell mode="login">
                {/* Header */}
                <div className="mb-8">
                    <h1 className="text-3xl font-extrabold text-ink-900 tracking-tight">{copy.welcome}</h1>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                    ...
                    {loginMethod === 'email' ? (
                        ...
                        <div>
                            <label className="block text-sm font-semibold text-ink-700 mb-2">{copy.phone}</label>
                    ...
                    <div className="flex justify-end">
                        <button type="button" onClick={() => setIsForgotModalOpen(true)} className="text-sm text-brand-600 hover:text-brand-700 font-semibold transition-colors">
                            {copy.forgot}
                        </button>
                    </div>
                ...
                <div className="space-y-3 text-center">
                    ...
                    <p className="text-sm">
                        <Link to="/verify-email" className="text-ink-400 hover:text-brand-600 font-medium transition-colors underline decoration-ink-200 underline-offset-4">
                            {copy.verifyEmail}
                        </Link>
                    </p>
                </div>
