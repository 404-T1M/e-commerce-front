...
function StatCard({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-2xl p-5 ${gradient} border border-white/20 shadow-sm card-hover`}
    >
...
export function DashboardPage() {
  ...
  const forbidden =
    isForbiddenError(overviewError) || isForbiddenError(chartError);
  if (forbidden) {
    ...
    locale === "ar"
      ? {
          ...
          quickLinks: {
            users: {
              label: "إدارة العملاء",
              desc: "عرض وتعديل والتحكم في حسابات العملاء",
            },
            admins: {
              label: "إدارة المشرفين",
              desc: "إضافة وإدارة حسابات المشرفين",
            },
            groups: {
              label: "مجموعات المشرفين",
              desc: "أدوار وصلاحيات المشرفين",
            },
      ...
      : {
          ...
          quickLinks: {
            users: {
              label: "Manage Users",
              desc: "View, edit, and control user accounts",
            },
            admins: {
              label: "Manage Admins",
              desc: "Add and manage admin accounts",
            },
            groups: {
              label: "Admin Groups",
              desc: "Role-based permission groups",
            },
            categories: {
              label: "Categories",
              desc: "Organize your product catalog",
            },
            products: {
              label: "Products",
  ...
  const quickLinks = QUICK_LINKS.map((link) => {
    const quickLink = copy.quickLinks[link.key as keyof typeof copy.quickLinks];
    return {
      ...link,
      label: quickLink?.label ?? link.key,
      desc: quickLink?.desc ?? "",
    };
  });
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 relative z-10">
              {firstCustomer &&
                (() => {
                  const userId =
                    firstCustomer.user?.id ?? firstCustomer.user?._id ?? "1";
                  const totalOrders =
                    (firstCustomer.activeOrders ?? 0) +
                    (firstCustomer.completedOrders ?? 0) +
                    (firstCustomer.cancelledOrders ?? 0);
                  return (
                    <div
                      key={userId}
              ...
              {secondCustomer &&
                (() => {
                  const userId =
                    secondCustomer.user?.id ?? secondCustomer.user?._id ?? "2";
                  const totalOrders =
                    (secondCustomer.activeOrders ?? 0) +
                    (secondCustomer.completedOrders ?? 0) +
                    (secondCustomer.cancelledOrders ?? 0);
                  return (
                    <div
                      key={userId}
              ...
              <div className="mt-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {runnerCustomers.map((c, idx) => {
                    const style =
                      runnerStyles[idx] ??
                      runnerStyles[runnerStyles.length - 1];
                    const userId = c.user?.id ?? c.user?._id ?? `runner-${idx}`;
                    const totalOrders =
                      (c.activeOrders ?? 0) +
                      (c.completedOrders ?? 0) +
                      (c.cancelledOrders ?? 0);
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3 min-w-0">
                          <div
                            className={cn(
                              "w-9 h-9 rounded-xl text-white text-xs font-bold flex items-center justify-center shadow-sm",
                              style.badge,
                            )}
                          >
                            #{rank}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {quickLinks.map(({ label, to, icon: Icon, desc }) => (
            <Link
              key={to}
              to={to}
              className="card p-4 flex items-start gap-4 card-hover group"
            >
